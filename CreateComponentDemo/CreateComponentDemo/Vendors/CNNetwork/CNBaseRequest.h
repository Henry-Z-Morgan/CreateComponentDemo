...
FOUNDATION_EXPORT NSString *const CNRequestValidationErrorDomain;

NS_ENUM(NSInteger) {
    CNRequestValidationErrorInvalidStatusCode = -8,
    CNRequestValidationErrorInvalidJSONFormat = -9,
};

///  HTTP Request method.
typedef NS_ENUM(NSInteger, CNRequestMethod) {
    CNRequestMethodGET = 0,
    CNRequestMethodPOST,
    CNRequestMethodHEAD,
    CNRequestMethodPUT,
    CNRequestMethodDELETE,
    CNRequestMethodPATCH,
};

///  Request serializer type.
typedef NS_ENUM(NSInteger, CNRequestSerializerType) {
    CNRequestSerializerTypeHTTP = 0,
    CNRequestSerializerTypeJSON,
};

///  Response serializer type, which determines response serialization process and
///  the type of `responseObject`.
typedef NS_ENUM(NSInteger, CNResponseSerializerType) {
    /// NSData type
    CNResponseSerializerTypeHTTP,
    /// JSON object type
    CNResponseSerializerTypeJSON,
    /// NSXMLParser type
    CNResponseSerializerTypeXMLParser,
};

///  Request priority
typedef NS_ENUM(NSInteger, CNRequestPriority) {
    CNRequestPriorityLow = -4L,
    CNRequestPriorityDefault = 0,
    CNRequestPriorityHigh = 4,
};
...
typedef void(^CNRequestCompletionBlock)(__kindof CNBaseRequest *request);

///  The CNRequestDelegate protocol defines several optional methods you can use
///  to receive network-related messages. All the delegate methods will be called
///  on the main queue.
@protocol CNRequestDelegate <NSObject>

@optional
///  Tell the delegate that the request has finished successfully.
///
///  @param request The corresponding request.
- (void)requestFinished:(__kindof CNBaseRequest *)request;

///  Tell the delegate that the request has failed.
///
///  @param request The corresponding request.
- (void)requestFailed:(__kindof CNBaseRequest *)request;

@end

///  The CNRequestAccessory protocol defines several optional methods that can be
...
@protocol CNRequestAccessory <NSObject>
...
///  CNBaseRequest is the abstract class of network request. It provides many options
///  for constructing request. It's the base class of `CNRequest`.
@interface CNBaseRequest : NSObject
...
///  `CNResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `CNResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<CNRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) CNRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) CNRequestCompletionBlock failureCompletionBlock;
...
///  The priority of the request. Effective only on iOS 8+. Default is `CNRequestPriorityDefault`.
@property (nonatomic) CNRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable CNRequestCompletionBlock)success
                              failure:(nullable CNRequestCompletionBlock)failure;
...
- (void)startWithCompletionBlockWithSuccess:(nullable CNRequestCompletionBlock)success
                                    failure:(nullable CNRequestCompletionBlock)failure;
